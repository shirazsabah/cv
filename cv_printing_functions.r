...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$memberships  <- read_gsheet(sheet_id = "memberships")
  } else {
    ...
    cv$memberships  <- readr::read_csv(paste0(data_location, "memberships.csv"), skip = 1)
  }
...
# Old code "- <i class='fa{icon}'></i> {contact}"
library(fontawesome)
print_contact_info <- function(cv){
  glue::glue_data(
    cv$contact_info,
    paste0("- ",fa_i(name = "{icon}", verify_fa=FALSE), " {contact}")
  ) %>% print()
...
print_member_info <- function(cv){
  glue::glue_data(
    cv$memberships,
    paste0("- <b>{org}:</b>"," {membno}")
  ) %>% print()
